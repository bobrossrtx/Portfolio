...
const buildStatsPayload = (login: string, user: GitHubGraphQLUser): GitHubStats => {
...
export const handler = async (event: { queryStringParameters?: Record<string, string> }) => {
  ...
  const login = event.queryStringParameters?.login ?? 'bobrossrtx';

  if (!token) {
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ error: 'Missing GitHub token on server.' }),
    };
  }
  ...
  if (!response.ok) {
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ error: 'Unable to load GitHub stats right now.' }),
    };
  }
  ...
  if (payload.errors?.length) {
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ error: payload.errors[0]?.message ?? 'Unable to load GitHub stats right now.' }),
    };
  }
  ...
  const stats = buildStatsPayload(login, payload.data.user);
